...
class Instance(object):
    ...
    def __init__(self, prox, logger, inst, run_type):
        self.proxmox = prox
        self.logger = logger
        self.inst = inst
    ...
    def Build(self):
        # First thing, query DB for any builds in flight, so we can push them
        # to the scheduler to be aware of.
        db = MySQLdb.connect(host =sql_host, user=sql_user, passwd=sql_pass, db=sql_db)
        cur = db.cursor(cursorclass =MySQLdb.cursors.DictCursor)
        query= "SELECT * FROM `instances` WHERE `state` IN (3);"
        cur.execute(query)
        if cur.rowcount == 0:
            resp= []
        else:
        ...
        if self.inst['backend_hypervisor'] is None:
            # Do a sleep first, this prevents racing of the PickNode function
            time.sleep(random.randint(0, 2000) / 1000)
            # Before we can build, get our host from the scheduler logic
            try:
                self.inst['backend_hypervisor'] = PickNode(self.proxmox, self.logger,resp,scheduler_logic,scheduler_op,self.inst['memory'],self.inst['cpu'])["name"]
            except Exception as e:
                    self.inst['state']= 50
                    self.Save_Instance()
                    raise Exception(
        ...
        while clone_trys < 5:
            ...
            try:
                if self.inst['backend_storage']:
                    self.event_id = self.proxmox.nodes(template_node).qemu(self.inst['template_id']).clone.post(newid =self.inst['backend_instance_id'],full=1,name=self.inst['hostname'],target=self.inst['backend_hypervisor'],storage=self.inst['backend_storage'])
                else:
                    self.event_id = self.proxmox.nodes(template_node).qemu(self.inst['template_id']).clone.post(newid =self.inst['backend_hypervisor'],full=1,name=self.inst['hostname'],target=self.inst['backend_hypervisor'])
            except Exception as e:
    ...
    def Resize(self):
        ...
        while cfg_trys < 5:
            try:
                cfg= self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).config.get()
            except Exception as e:
                self.logger.debug(
        ...
        while cfg_trys < 5:
            try:
                self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).config.put(memory =self.inst['memory'],cores=self.inst['cpu'])
            except Exception as e:
                self.logger.debug(
        ...
        while cfg_trys < 5:
            try:
                self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).resize.put(disk =bootdisk,size=str(self.inst['disk'])+"G")
            except Exception as e:
    ...
    def Userdata(self):
        ...
        if ssh_public_key != "" and ssh_public_key is not None:
            ...
            userdata= str(resp[0]['userdata']).replace("{PSCHED-SSH-KEY}", ssh_public_key)

        # Get our current userdata for the instance, and apply our stuff
        cfg_trys= 0
        while cfg_trys < 5:
            try:
                tmp_desc= self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).config.get()['description'] + "\n" + ud_start_flag + "\n" + userdata + "\n" + ud_end_flag
            except Exception as e:
                self.logger.debug(
        ...
        while cfg_trys < 5:
            try:
                self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).config.put(description =tmp_desc)
            except Exception as e:
                self.logger.debug(
    ...
    def Start(self):
        ...
        while prox_trys < 5:
            try:
                self.event_id= self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).status.start.post()
            except Exception as e:
    ...
    def GetIP(self):
        ...
        while master_loop < 60:
            ...
            while prox_trys < 5:
                try:
                    cfg= self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).config.get()['description']
                except Exception as e:
                    self.logger.debug(
    ...
    def BuildStatus(self):
        ...
        while True:
            ...
            try:
                ...
                msgs = ["Unable to connect", "No existing session","Authentication failed"]
                for error in msgs:
                    if error in str(e):
                        ssh.close()  # Cleanup
                ...
                while True:
                    ...
                    try:
                        stdin, stdout, stderr= ssh.exec_command("cat " + build_flagfile, timeout =3)
    ...
    def Shutdown(self):
        ...
        while prox_trys < 5:
            try:
                self.event_id= self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).status.stop.post()
            except Exception as e:
                self.logger.debug(
    ...
    def Destroy(self):
        ...
        while rm_master < 3:
            ...
            while rm_trys < 5:
                try:
                    self.event_id= self.proxmox.nodes(self.inst['backend_hypervisor']).qemu(self.inst['backend_instance_id']).delete()
                except Exception as e:
                    self.logger.debug(
